...
public class PartieService {
    ...
    public void updatePartie(UUID uuid, PartieForm partieForm) {
        Partie partie = partieRepository.findPartieByUuid(uuid).orElseThrow();
    ...
    private void verification(PartieForm partieForm) {
        ...
        if (!joueurService.existeJoueur(partieForm.getPreneurId())) {
            throw new TdaException("Joueur avec id " + partieForm.getPreneurId() + " n'existe pas !!!");
        }

        if (!joueurService.existeJoueur(partieForm.getAppelId())) {
            throw new TdaException("Joueur avec id " + partieForm.getAppelId() + " n'existe pas !!!");
        }

        if (!joueurService.existeJoueur(partieForm.getMortId())) {
            throw new TdaException("Joueur avec id " + partieForm.getMortId() + " n'existe pas !!!");
        }

        if (!joueurService.existeJoueur(partieForm.getPetitAuBoutId())) {
            throw new TdaException("Joueur avec id " + partieForm.getPetitAuBoutId() + " n'existe pas !!!");
        }
